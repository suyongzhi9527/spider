...
def get_page_offer(keyword, pageNo):
    url = "https://data.p4psearch.1688.com/data/ajax/get_premium_offer_list.json?beginpage=%d&keywords=%s" % (
        pageNo, keyword)
    ...
    for offer in offerResult:
        obj = dict()
        # print(offer['attr']['id'])
        obj['id'] = offer['attr']['id']
        # print(offer['title'])
        obj['title'] = str(offer['title']).replace(
            '<font color=red>', '').replace('</font>', '')
        # print(offer['attr']['company']['shopRepurchaseRate'])
        obj['shopRepurchaseRate'] = str(
            offer['attr']['company']['shopRepurchaseRate'])
...
def wirte_excel_xlsx(path, sheet_name, value):
    index = len(value)
...
def main(keyword, page):
    ...
    # get_premium_offer_list(keyword, page)
    wirte_excel_xlsx('data.xlsx', '数据', offer_list)
