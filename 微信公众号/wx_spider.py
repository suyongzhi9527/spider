...
def get_wx_url_by_sougou_search_html(sougou_search_html):
    ...
    return doc('li[id=sogou_vr_11002301_box_0]')('dd')('a').attr('href')
...
def parse_wx_articles_by_html(selenium_html):
    ...
    return doc('div[class="rich_media_area_primary_inner"]')
...
def parse_one_article(article):
    ...
    # 获取标题
    title = article('h2[class="rich_media_title"]').text().strip()
    ###log(u'标题是： %s' % title)
    # 获取标题对应的地址
    # url = 'http://mp.weixin.qq.com' + article('h4[class="weui_media_title"]').attr('hrefs')
    # log(u'地址为： %s' % url)
    # 获取概要内容
    summary = article('.rich_media_content').text()
    log(u'文章简述： %s' % summary)
    # 获取文章发表时间
    date = article('.rich_media_meta rich_media_meta_text').text().strip()
    log(u'发表时间为： %s' % date)
    # 获取封面图片
...
def run(keywords):
    ...
    # 搜狐微信搜索链接入口
    sogou_search_url = 'http://weixin.sogou.com/weixin?type=1&query=%s&ie=utf8&s_from=input&_sug_=n&_sug_type_=' % parse.quote(keywords)
    ...
    wx_url = 'https://weixin.sogou.com' + wx_url
